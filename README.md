...
- Please note that wile we provide examples for directories, you may download and extract the files in this repo to a location of your own so long as the directory you point to in the below procedures is the exact folder that contains `Figure3.py` and `Figure3_MAC.py`. In the below examples these folders may change depending on how each OS extracts te zip files, but the target directories are those that directly contain the aforementioned files.
